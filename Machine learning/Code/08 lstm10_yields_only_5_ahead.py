...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 26 17:58:27 2020
...
X = X.iloc[:,X.columns.str.contains('J')].copy()
...
# checkpoint
modelCheckpoint1 = ModelCheckpoint(filepath = model_folder + 'best_vanilla_lstm 10 5 ahead.hdf5',  save_best_only = True)
...
results2 = lstm_vanilla.evaluate(X_test, y_test, batch_size = batch_size)
...
l = KerasRegressor(build_fn = vanilla_LSTM,nb_features = int(X.shape[1]/time_steps),time_steps = len(times),output_dim = y.shape[1],      
            learning_rate = 0.01,
            size = 13,
            epochs = 200,
            batch_size = 100,
...
cv2 = cross_val_score(l,X_train,y_train,cv = cpcv,n_jobs = 3)
print(cv2)
# 1.984468223342184e-06
...
cv3 = cross_val_score(l,X_train,y_train,cv = cpcv,n_jobs = 3)
print(cv3)
#2.584592091354294e-06
